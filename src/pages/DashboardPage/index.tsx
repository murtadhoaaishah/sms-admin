...
const DashboardPage = () => {
  ...
  const tableData = [
    {
      ...
      paidvia: "Remita",
      status: (
        <span className="success px-[10px] py-[4px] text-[#15A336] rounded-lg">
          success
        </span>
      ),
      date: "Oct. 14, 2021",
  ...
  const statistics = [
    {
      ...
      number: "32,450",
      percentage: (
        <div className=" percentage-arrow flex flex-row items-center rounded-lg  px-[8px]">
    ...
    },
    {
      ...
      number: "32,450",
      percentage: (
        <div className=" red-percentage-arrow flex flex-row items-center rounded-lg  px-[8px]">
  ...
  const communication = [
    {
      icon: <img src="/vectors/telephone.svg" alt="" />,
    ...
    {
      icon: <img src="/message.svg" alt="" />,
      action: (
        <span className="line-through">
          Schedule a conference call/or send message
        </span>
      ),
      details: "We use these details to match you with the right students",
    },
  ...
  return (
    <DashboardLayout
      title={
        <p className="header">
          <span>Welcome,</span> <span className="gwen">Gwen School</span> 👋
        </p>
      }
    >
      <main className="dashboard-page">
        <section className="statistics">
          {statistics.map(
            ({ heading, number, percentage }, statisticsIndex) => (
              <div
                key={`statistics_${statisticsIndex}`}
                className={`data flex  justify-left items-center ${
                  statisticsIndex == statistics.length - 1 && " border-0"
                }`}
              >
                <span
                  className={`py-3 ${
                    statisticsIndex == 0
                      ...
                      : statisticsIndex == statistics.length - 1
                      ? "flex self-end flex-col"
                      : ""
                  }`}
                >
        ...
        <aside className="communication-wrapper">
          {communication.map(
            ({ icon, action, details }, communicationIndex) => (
              <div
                key={`communication_${communicationIndex}`}
